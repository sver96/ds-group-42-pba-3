...
for (col_name in colnames(train_10)) {
  ...
  train_10[[col_name]][na_indices] <- NA
}

na_counts <- colSums(is.na(train_10))
na_counts
...
for (col_name in colnames(train_40)) {
  n_na <- round(prop_na * n_rows)
  na_indices <- sample(1:n_rows, size = n_na, replace = FALSE)
  train_40[[col_name]][na_indices] <- NA
}

na_counts <- colSums(is.na(train_40))
na_counts
...
for (col_name in colnames(train_70)) {
  n_na <- round(prop_na * n_rows)
  na_indices <- sample(1:n_rows, size = n_na, replace = FALSE)
  train_70[[col_name]][na_indices] <- NA
}

na_counts <- colSums(is.na(train_70))
na_counts
